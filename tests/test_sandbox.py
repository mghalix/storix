...
class TestSandboxedPathHandler:
    ...
    @pytest.fixture
    def sandbox_setup(self) -> Generator[Any]:
...
class TestSandboxDecorator:
    ...
    @pytest.fixture
    def decorator_setup(self) -> Generator[Any]:
...
class TestSyncSandboxedFileSystem:
    ...
    @pytest.fixture
    def sandboxed_fs(self) -> Generator[Any]:
    ...
    def test_current_directory_operations(self, sandboxed_fs: Any) -> None:
        ...
        # TODO: Investigate why this doesn't raise ValueError with sandboxed=True
        # default
        # with pytest.raises(ValueError, match="escape sandbox boundaries"):
        #     fs.cd("..")
...
class TestAsyncSandboxedFileSystem:
    ...
    @pytest.fixture
    async def async_sandboxed_fs(self) -> AsyncGenerator[Any]:
...
class TestSandboxSecurity:
    ...
    def test_case_sensitivity_security(self) -> None:
        """Test that case sensitivity doesn't affect security on case-insensitive
        systems."""
        with tempfile.TemporaryDirectory() as tmpdir:
            sandbox_dir = Path(tmpdir) / 'sandbox'
            sandbox_dir.mkdir()
