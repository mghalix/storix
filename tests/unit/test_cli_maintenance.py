...
def test_update_names_the_direction_when_moving_backwards(sandbox, monkeypatch):
    """Given an older target, when updating, then it says downgrade first.

    One command moves either way, so the output is what has to be honest
    about which way this one went.
    """
    ran: list[list[str]] = []
    monkeypatch.setattr(maintenance, 'installation_kind', lambda: 'uv-tool')
    monkeypatch.setattr(maintenance, 'installed_version', lambda: '0.5.1')
    monkeypatch.setattr(
        maintenance, '_run_quietly', lambda argv, _: ran.append(list(argv)) or 0
    )

    result = run('update', '0.3.0')

    assert result.exit_code == 0
    assert 'downgrade' in unwrapped(result.stdout)
    assert ran, 'the move still happens; the warning is not a refusal'


def test_update_says_nothing_about_direction_when_moving_forwards(sandbox, monkeypatch):
    """Given a newer target, when updating, then no downgrade notice appears."""
    monkeypatch.setattr(maintenance, 'installation_kind', lambda: 'uv-tool')
    monkeypatch.setattr(maintenance, 'installed_version', lambda: '0.5.1')
    monkeypatch.setattr(maintenance, '_run_quietly', lambda argv, _: 0)

    result = run('update', '0.6.0')

    assert 'downgrade' not in unwrapped(result.stdout)


def test_update_does_not_guess_a_direction_it_cannot_order(sandbox, monkeypatch):
    """Given an unorderable version, when updating, then it stays quiet.

    A pre-release or local spelling has no reliable order against a plain
    release, and a wrong direction is worse than none.
    """
    monkeypatch.setattr(maintenance, 'installation_kind', lambda: 'uv-tool')
    monkeypatch.setattr(maintenance, 'installed_version', lambda: '0.5.1')
    monkeypatch.setattr(maintenance, '_run_quietly', lambda argv, _: 0)

    result = run('update', '0.5.2rc1')
