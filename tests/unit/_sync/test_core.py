...
def test_provision_on_capable_backend_reports_present():
    # Given a session over a backend whose root is always present
    fs = Storix(MemoryBackend())
    # When provision runs, the capable backend answers directly
    assert fs.provision() is False


def test_provision_unsupported_backend_names_provider_tooling():
...
def test_provision_targets_the_base_backend_under_layers():
...
def test_download_writes_the_whole_file(tmp_path):
    """Given a file, when downloaded to a sink, then the bytes match."""
    fs = Storix(MemoryBackend())
    payload = bytes(range(256)) * 4096
    fs.echo(payload, '/big.bin')
    dest = tmp_path / 'out.bin'

    with dest.open('wb') as handle:
        written = fs.download('/big.bin', handle)
...
def test_download_in_parallel_ranges_matches_sequential(tmp_path, monkeypatch):
    """Given several ranges, when downloaded, then the file is assembled in order."""
    from storix._sync import core
    ...
    with dest.open('wb') as handle:
        written = fs.download('/big.bin', handle, ranges=4, chunk_size=1024)

    assert written == len(payload)
    assert dest.read_bytes() == payload


def test_download_to_a_buffer_falls_back_to_sequential():
    """Given a sink with no descriptor, when downloaded, then it still works."""
    fs = Storix(MemoryBackend())
    fs.echo(b'payload', '/small.bin')
    sink = io.BytesIO()

    written = fs.download('/small.bin', sink, ranges=4)

    assert written == 7
    assert sink.getvalue() == b'payload'


def test_download_rejects_a_directory(tmp_path):
    """Given a directory, when downloaded, then it raises."""
    fs = Storix(MemoryBackend())
    fs.mkdir('/dir')

    with (tmp_path / 'out.bin').open('wb') as handle, pytest.raises(IsADirectoryError):
        fs.download('/dir', handle)


@pytest.mark.parametrize('ranges', [0, -1])
def test_download_rejects_a_non_positive_range_count(tmp_path, ranges):
    ...
    with (tmp_path / 'out.bin').open('wb') as handle, pytest.raises(ValueError):
        fs.download('/f.bin', handle, ranges=ranges)


def test_download_below_the_range_threshold_stays_sequential(tmp_path):
    """Given a small file, when ranges are asked for, then one stream is used."""
    fs = Storix(MemoryBackend())
    fs.echo(b'small', '/small.bin')

    with (tmp_path / 'out.bin').open('wb') as handle:
        written = fs.download('/small.bin', handle, ranges=8)

    assert written == 5
    assert (tmp_path / 'out.bin').read_bytes() == b'small'


def test_download_into_a_transforming_sink_stays_sequential(tmp_path, monkeypatch):
    ...
    with gzip.open(archive, 'wb') as sink:
        fs.download('/big.bin', sink, ranges=4)

    assert gzip.decompress(archive.read_bytes()) == payload


def test_echo_hands_back_the_version_stat_reported():
    """Given a version from stat, when echoing with it, then the write lands."""
    fs = Storix(MemoryBackend())
    fs.echo('one', '/a.txt')

    version = (fs.stat('/a.txt')).version
    fs.echo('two', '/a.txt', if_match=version)

    assert fs.cat('/a.txt') == b'two'


def test_echo_refuses_a_write_whose_version_moved():
    ...
    fs = Storix(MemoryBackend())
    fs.echo('one', '/a.txt')
    stale = (fs.stat('/a.txt')).version
    fs.echo('two', '/a.txt')

    with pytest.raises(PreconditionFailedError):
        fs.echo('three', '/a.txt', if_match=stale)

    assert fs.cat('/a.txt') == b'two'


def test_echo_does_not_retry_a_failed_precondition():
    ...
    assert (fs.stat('/a.txt')).version != stale
    assert fs.cat('/a.txt') == b'two'


def test_echo_creates_exclusively_when_asked():
    """Given an occupied path, when creating exclusively, then it is refused."""
    fs = Storix(MemoryBackend())
    fs.echo('first', '/a.txt')

    with pytest.raises(PreconditionFailedError):
        fs.echo('second', '/a.txt', if_match=IF_MATCH_ABSENT)

    fs.echo('mine', '/b.txt', if_match=IF_MATCH_ABSENT)

    assert fs.cat('/a.txt') == b'first'
    assert fs.cat('/b.txt') == b'mine'


def test_echo_rejects_a_precondition_on_append():
    ...
    with pytest.raises(ValueError, match='if_match'):
        fs.echo('two', '/a.txt', mode='a', if_match=version)


def test_a_directory_reports_no_version():
...
def test_a_path_object_destination_carries_the_assertion_too():
    ...
    with pytest.raises(NotADirectoryError):
        fs.cp('/a.txt', StorixPath('/nodir/'))

    assert not fs.exists('/nodir')


def test_a_derived_path_makes_no_assertion():
    ...
    fs.cp('/a.txt', StorixPath('/b.txt/x').parent)

    assert fs.cat('/b.txt') == b'one'


def test_a_shallow_glob_does_not_reach_below_what_it_can_match():
    ...
    """
    fs = Storix(MemoryBackend())
    fs.mkdir('/sub/deeper/deepest', parents=True)
    fs.echo('x', '/a.txt')
    fs.echo('x', '/sub/deeper/deepest/buried.txt')

    matched = [str(p) for p in fs.glob('*')]
...
def test_a_recursive_glob_still_reaches_every_depth():
    ...
    found = [str(p) for p in fs.glob('**/*.txt')]

    assert '/sub/deeper/deepest/buried.txt' in found
    assert '/a.txt' in found


def test_glob_depth_is_derived_from_the_pattern():
    ...
    """
    from storix._sync.core import _glob_depth
