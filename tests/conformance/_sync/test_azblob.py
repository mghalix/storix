...
# source of truth: tests/conformance/_async/test_azblob.py
...
def test_make_url_signs_locally(monkeypatch: pytest.MonkeyPatch):
    fs = _blob()

    def stat(_path: P) -> RawStat:
        return _file_stat()

    monkeypatch.setattr(fs, 'stat', stat)

    url = fs.make_url(P('/docs/a.txt'))

    assert url.startswith('https://acct.blob.core.windows.net/mycontainer/docs/a.txt?')
    assert 'sig=' in url


def test_make_url_honors_root_and_endpoint(monkeypatch: pytest.MonkeyPatch):
...
def test_make_url_rejects_a_directory(monkeypatch: pytest.MonkeyPatch):
    fs = _blob()
