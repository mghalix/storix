...
def test_read_range_yields_only_the_range(backend: StorageBackend):
    put(backend, '/a.txt', b'0123456789')
    chunks = list(backend.read_range(P('/a.txt'), offset=3, length=4))
    assert b''.join(chunks) == b'3456'


def test_read_range_stops_at_the_end_of_the_file(backend: StorageBackend):
    put(backend, '/a.txt', b'0123456789')
    chunks = list(backend.read_range(P('/a.txt'), offset=8, length=99))
    assert b''.join(chunks) == b'89'


def test_read_range_past_the_end_is_empty(backend: StorageBackend):
    put(backend, '/a.txt', b'0123456789')
    chunks = list(backend.read_range(P('/a.txt'), offset=99, length=4))
    assert chunks == []


def test_read_range_honors_chunk_size(backend: StorageBackend):
    put(backend, '/a.txt', b'0123456789')
    chunks = list(backend.read_range(P('/a.txt'), offset=1, length=8, chunk_size=3))
    assert b''.join(chunks) == b'12345678'
    assert all(0 < len(chunk) <= 3 for chunk in chunks)


def test_read_range_ranges_reassemble_the_whole_file(backend: StorageBackend):
    """Every backend's ranges must tile the file exactly, native or emulated."""
    put(backend, '/a.txt', b'0123456789')
    spans = [(0, 4), (4, 4), (8, 4)]
    parts = [
        b''.join(list(backend.read_range(P('/a.txt'), offset=o, length=n)))
        for o, n in spans
    ]
...
def test_read_range_rejects_a_negative_span(
    backend: StorageBackend, offset: int, length: int
):
    put(backend, '/a.txt', b'x')
    with pytest.raises(ValueError, match='must not be negative'):
        list(backend.read_range(P('/a.txt'), offset=offset, length=length))
...
def test_delete_tree_spares_a_name_sharing_sibling(backend: StorageBackend):
    ...
    """
    backend.make_dir(P('/d'), parents=True)
    put(backend, '/d/gone.txt', b'x')
    backend.make_dir(P('/d2'), parents=True)
    put(backend, '/d2/kept.txt', b'y')

    backend.delete_tree(P('/d'))

    assert not backend.exists(P('/d'))
    assert backend.read(P('/d2/kept.txt')) == b'y'


def test_delete_tree_missing_raises(backend: StorageBackend):
    with pytest.raises(PathNotFoundError):
        backend.delete_tree(P('/missing'))


def test_delete_tree_on_a_file_raises(backend: StorageBackend):
    put(backend, '/a.txt', b'x')
    with pytest.raises(NotADirectoryError):
        backend.delete_tree(P('/a.txt'))
    assert backend.read(P('/a.txt')) == b'x'
...
def test_a_conditional_write_wins_on_the_version_it_read(
    backend: StorageBackend,
):
    ...
    backend.write_stream(P('/cw.txt'), _astream(b'one'), mode='w', content_type=None)
    version = (backend.stat(P('/cw.txt'))).version
    ...
    assert backend.read(P('/cw.txt')) == b'two'


def test_a_conditional_write_loses_on_a_stale_version(backend: StorageBackend):
    ...
    if not backend.capabilities.conditional_writes:
        pytest.skip('backend does not advertise conditional_writes')
    backend.write_stream(P('/stale.txt'), _astream(b'one'), mode='w', content_type=None)
    stale = (backend.stat(P('/stale.txt'))).version
    backend.write_stream(P('/stale.txt'), _astream(b'two'), mode='w', content_type=None)
    ...
    assert backend.read(P('/stale.txt')) == b'two'


def test_an_exclusive_create_refuses_an_occupied_path(backend: StorageBackend):
    ...
    backend.write_stream(P('/x.txt'), _astream(b'first'), mode='w', content_type=None)
    ...
    assert backend.read(P('/x.txt')) == b'first'


def test_an_exclusive_create_takes_a_free_path(backend: StorageBackend):
    ...
    assert backend.read(P('/free.txt')) == b'mine'


def test_a_precondition_is_refused_on_append(backend: StorageBackend):
    ...
    backend.write_stream(P('/app.txt'), _astream(b'one'), mode='w', content_type=None)
...
def test_a_precondition_form_a_backend_lacks_is_refused(
    backend: StorageBackend,
):
