"""Stream a live FFmpeg producer into the configured Storix backend."""
...
from storix.aio import ObservabilityLayer, Storix, TransferEvent, get_storage
...
async def main() -> None:
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn(f'[bold]{label:<7}[/bold]'),
        TextColumn('streaming'),
        TextColumn('{task.completed:,.0f} bytes'),
        TimeElapsedColumn(),
        transient=True,
        console=console,
    ) as progress:
        ...
        def on_transfer(event: TransferEvent) -> None:
            progress.update(task, completed=event.transferred)

        session = base.with_layer(
            ObservabilityLayer,
            sink=on_transfer,
        )
