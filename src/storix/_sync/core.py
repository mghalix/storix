...
class Storix:
    ...
    def glob(
        ...
        for entry in self.walk(base, all=all, max_depth=_glob_depth(pattern)):
    ...
    def download(
        ...
        *,
        ...
        if fd is None or count == 1:
            written = 0
            for chunk in self._backend.read_stream(target, chunk_size=chunk_size):
                dest.write(chunk)
                written += len(chunk)
            return written
    ...
    def _fetch_span(
        ...
        position = offset
        for chunk in self._backend.read_range(
