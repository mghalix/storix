...
class OpendalBackend(BackendBase):
    ...
    def __init__(
        ...
        **options: str,
        ...
        self.capabilities = Capabilities(
            ...
            # opendal serves a range with a ranged request (a Range header
    ...
    async def read_range(
        ...
        *,
        ...
        """Seek to the range and stream it, one ranged request.
        ...
        if raw.kind is PathKind.DIRECTORY:
            raise IsADirectoryError(path)
        remaining = min(length, max(0, raw.size - offset))
        if not remaining:
            return
        # explicit close, not `async with`: opendal's AsyncFile.__aexit__
        # stub is mistyped as returning None instead of an awaitable
    ...
    async def write_stream(
        ...
        *,
        ...
        if raw is not None and mode == 'a':
            ...
            # mode='a' rejects a precondition, so none is passed here
    ...
    async def copy(self, src: PurePosixPath, dst: PurePosixPath) -> None:
        ...
        if not self._native_copy:
            ...
            return
        await self._reject_directory(dst)
        try:
            await self._op.copy(self._key(src), self._key(dst))
        except ope.NotFound as exc:
            await self._reject_directory(src)
            raise self._error(exc, src) from exc
        except _OPENDAL_ERRORS as exc:
            raise self._error(exc, src) from exc

    async def _reject_directory(self, path: PurePosixPath) -> None:
        ...
        """
        try:
            await self._op.stat(self._dir_key(path))
        except ope.NotFound:
            return
        except _OPENDAL_ERRORS as exc:
            raise self._error(exc, path) from exc
    ...
    async def du(self, path: PurePosixPath) -> int:
        ...
        total = 0
        try:
            entries = await self._op.list(self._dir_key(path), recursive=True)
            async for entry in entries:
