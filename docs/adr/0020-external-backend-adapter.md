...
- [ ] Spike first: `src/storix/_async/backends/opendal.py` `OpendalBackend` over
...
- [ ] Capabilities: advertise `presigned_urls` (opendal `presign`) where the
      service supports it; gate `make_url` on it. Metadata where available.
- [ ] Factory: register `opendal` service resolvers; lazy import guarded to
      raise the typed missing-extra error (`storix[opendal]`).
- [ ] `pyproject.toml`: `storix[opendal]` optional-dependency group.
- [ ] Write the `_sync` twin by hand (`opendal.Operator`); do NOT run the async
